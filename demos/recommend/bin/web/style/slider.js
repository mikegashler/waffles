...
function slider (a_init, a_tpl) {
	...
	e_input.onchange = f_onTextChange;
	g_textBoxesActive = true;
	if (!window.A_TBTOSLIDERS)
		window.A_TBTOSLIDERS = [];
	window.A_TBTOSLIDERS[e_input.name] = this;

	// uncheck the box
	var e_checkbox = this.s_form == null
		? get_element(this.s_checkname)
		: document.forms[this.s_form]
			? document.forms[this.s_form].elements[this.s_checkname]
			: null;
	e_checkbox.checked = false;
}

function f_onTextChange (e) {
	if (!g_textBoxesActive) return;
	if (!e && window.event) e = window.event;
	if (e.target) targ = e.target;
	else if (e.srcElement) targ = e.srcElement;
	o_sldr = window.A_TBTOSLIDERS[targ.name];
	o_sldr.f_setValue (targ.value, true);
}
...
function f_sliderSetValue (n_value, b_noInputCheck) {
	...
	var e_checkbox;
	if (this.s_form == null) {
		e_checkbox = get_element(this.s_checkname);
		if (!e_checkbox)
			return f_sliderError(this.n_id, "Expected an element with ID='" + this.s_checkname + "'.");
	}
	else {
		var e_form = document.forms[this.s_form];
		if (!e_form)
			return f_sliderError(this.n_id, "Can not find the form with NAME='" + this.s_form + "'.");
		e_checkbox = e_form.elements[this.s_checkname];
		if (!e_checkbox)
			return f_sliderError(this.n_id, "Can not find the input with NAME='" + this.s_checkname + "'.");
	}
