...
UsageNode* makeLearnUsageTree()
{
	...
	UsageNode* pTrain = pRoot->add("train <options> [dataset] <data_opts> [algorithm]", "Trains a supervised learning algorithm. The trained model-file is printed to stdout. (Typically, you will want to pipe this to a file.)");
	{
		...
		pTrain->add("[dataset]", "The filename of a dataset in \".arff\" format.");
	...
	UsageNode* pTransduce = pRoot->add("transduce <options> [labeled-set] <data_opts1> [unlabeled-set] <data_opts2> [algorithm]", "Predict labels for [unlabeled-set] based on the examples in [labeled-set]. For most algorithms, this is the same as training on [labeled-set] and then predicting labels for [unlabeled-set]. Some algorithms, however, have no models. These can transduce, even though they cannot be trained. The predicted labels are printed to stdout as a \".arff\" file.");
	{
		...
		pOpts->add("-seed [value]", "Specify a seed for the random number generator. (Use this option to ensure that your results are reproduceable.)");
		pTransduce->add("[labeled-set]", "The filename of a dataset in \".arff\" format. The labels in this dataset are used to infer labels for the unlabeled set.");
	...
	UsageNode* pTransAcc = pRoot->add("transacc <options> [training-set] <data_opts1> [test-set] <data_opts2> [algorithm]", "Measure the transductive accuracy of [algorithm] with respect to the specified training and test sets. Results are printed to stdout for each dimension in the label vector. Predictive accuracy is reported for nominal label dimensions, and mean-squared-error is reported for continuous label dimensions.");
	{
		...
		pOpts->add("-seed [value]", "Specify a seed for the random number generator. (Use this option to ensure that your results are reproduceable.)");
		pTransAcc->add("[training-set]", "The filename of a dataset in \".arff\" format. The labels in this dataset are used to infer labels for the unlabeled set.");
	...
	UsageNode* pCV = pRoot->add("crossvalidate <options> [dataset] <data_opts> [algorithm]", "Perform cross-validation with the specified dataset and algorithm. Results are printed to stdout. (Supports model-free algorithms too.)");
	{
		...
		pCV->add("[dataset]", "The filename of a dataset in \".arff\" format.");
	...
	UsageNode* pPR = pRoot->add("precisionrecall <options> [dataset] <data_opts> [algorithm]", "Compute the precision/recall for a dataset and algorithm");
	{
		...
		pPR->add("[dataset]", "The filename of a dataset in \".arff\" format.");
...
UsageNode* makeAlgorithmUsageTree()
{
	...
	UsageNode* pDT = pRoot->add("decisiontree <options>", "A decision tree.");
	{
		...
		pOpts->add("-maxlevels [n]", "When building the tree, if a path from the root to the current node contains n nodes (including the root), it will stop trying to divide the data and will create a leaf node.  This means that there will be at most n-1 splits before a decision is made.  This crudely limits overfitting, and so can be helpful on small data sets.  It can also make the resulting trees easier to interpret.  If set to 0, then there is no maximum. 0 is the default.");
	}
...
UsageNode* makeTransformUsageTree()
{
	...
	{
		pCumCols->add("[dataset]", "The filename of a dataset in ARFF format.");
		pCumCols->add("[column-list]", "A comma-separated list of zero-indexed columns to transform. A hypen may be used to specify a range of columns. Example: 0,2-5,7");
	}
	...
	{
		pDropCols->add("[column-list]", "A comma-separated list of zero-indexed columns to drop. A hypen may be used to specify a range of columns. Example: 0,2-5,7");
	}
	...
	{
		pPowerCols->add("[dataset]", "The filename of a dataset in ARFF format.");
		pPowerCols->add("[column-list]", "A comma-separated list of zero-indexed columns to transform. A hypen may be used to specify a range of columns. Example: 0,2-5,7");
		pPowerCols->add("[exponent]", "An exponent value, such as 0.5, 2, etc.");
	}
	UsageNode* pPCA = pRoot->add("pca [dataset] [target_dims] <options>", "Projects the data into the specified number of dimensions with principle component analysis. (Prints results to stdout. The input file is not modified.)");
	{
		...
		pOpts->add("-seed [value]", "Specify a seed for the random number generator.");
		pOpts->add("-roundtrip [filename]", "Do a lossy round-trip of the data and save the results to the specified file.");
		pOpts->add("-eigenvalues [filename]", "Save the eigenvalues to the specified file.");
		pOpts->add("-components [filename]", "Save the centroid and principal component vectors (in order of decreasing corresponding eigenvalue) to the specified file.");
	...
	{
	  pRotate->add("[dataset]", "The filename of a dataset in ARFF format.");
	  pRotate->add("[col_x]", "The zero-based index of an attribute to serve as the x coordinate in the plane of rotation.  Rotation from x to y will be 90 degrees. col_x must be a real-valued attribute.");
	  pRotate->add("[col_y]", "The zero-based index of an attribute to serve as the y coordinate in the plane of rotation.  Rotation from y to x will be 270 degrees. col_y must be a real-valued attribute.");
	  pRotate->add("[angle_degrees]","The angle in degrees to rotate around the origin in the col_x,col_y plane.");
	}
	UsageNode* pScaleCols = pRoot->add("scalecolumns [dataset] [column-list] [scalar]", "Multiply the values in the specified columns by a sacalr.");
	{
		pScaleCols->add("[dataset]", "The filename of a dataset in ARFF format.");
		pScaleCols->add("[column-list]", "A comma-separated list of zero-indexed columns to transform. A hypen may be used to specify a range of columns. Example: 0,2-5,7");
		pScaleCols->add("[scalar]", "A scalar value.");
	}
	...
	{
		pShiftCols->add("[dataset]", "The filename of a dataset in ARFF format.");
		pShiftCols->add("[column-list]", "A comma-separated list of zero-indexed columns to transform. A hypen may be used to specify a range of columns. Example: 0,2-5,7");
		pShiftCols->add("[offset]", "A positive or negative value to add to the values in the specified columns.");
	}
...
UsageNode* makePlotUsageTree()
{
	...
	{
		pOL->add("[png1]", "The filename of an image in png format.");
		pOL->add("[png2]", "The filename of an image in png format.");
		UsageNode* pOpts = pOL->add("<options>");
		pOpts->add("-out [filename]", "Specify the name of the output file. (The default is plot.png.) It should have the .png extension because other image formats are not yet supported.");
		pOpts->add("-backcolor [hex]", "Specify the six-digit hexadecimal representation of the background color. (This color will be treated as being transparent in [png2]. The default is ffffff (white).");
		pOpts->add("-tolerance [n]", "Specify the tolerance (an integer). The default is 0. If a larger value is specified, then colors close to the background color will also be treated as transparent in [png2].");
	}
...
UsageNode* makeNeighborUsageTree()
{
	...
	UsageNode* pMan = pRoot->add("manifold <options> [t] [radius] [thresh]", "An intelligent neighbor-finder that finds neighborhoods with aligned tangent hyperplanes.");
	{
		UsageNode* pOpts = pMan->add("<options>");
		UsageNode* pCC = pOpts->add("-cyclecut [thresh]", "Use CycleCut to break shortcuts and cycles.");
		pCC->add("[thresh]", "The threshold cycle-length for bad cycles.");
		pMan->add("[t]", "The number of dimensions in the tangent hyperplanes.");
		pMan->add("[radius]", "The maximum distance to a neighbor.");
		pMan->add("[thresh]", "The minimum value for the squared cosine of the angle between the tangent spaces of two neighbors.");
	}
...
UsageNode* makeRecommendUsageTree()
{
	...
	UsageNode* pCV = pRoot->add("crossvalidate <options> [sparse-data] [collab-filter]", "Measure accuracy using cross-validation");
	{
		UsageNode* pOpts = pCV->add("<options>");
		pOpts->add("-seed [value]", "Specify a seed for the random number generator.");
		pOpts->add("-folds [n]", "Specify the number of folds. The default is 2.");
		pOpts->add("-maxrecs [n]", "Evaluate only the [n] recommendations for each user with the highest predicted rating. The default is 1000000.");
		pCV->add("[sparse-data]", "The filename of a sparse matrix where rows indicate users, columns indicate items, and elements in the matrix indicate ratings. Alternatively, you may specify the filename of a 3-column dense ARFF file where each row specifies a user-id, item-id, and rating.");
	}
	...
	UsageNode* pPR = pRoot->add("precisionrecall <options> [sparse-data] [collab-filter]", "Compute precision-recall data");
	{
		...
		pPR->add("[sparse-data]", "The filename of a sparse matrix where rows indicate users, columns indicate items, and elements in the matrix indicate ratings. Alternatively, you may specify the filename of a 3-column dense ARFF file where each row specifies a user-id, item-id, and rating.");
	}
	UsageNode* pROC = pRoot->add("roc <options> [sparse-data] [collab-filter]", "Compute data for an ROC curve. (The area under the curve will appear in the comments at the top of the data.)");
	{
		...
		pOpts->add("-seed [value]", "Specify a seed for the random number generator.");
		pOpts->add("-ideal", "Ignore the model and compute ideal results (as if the model always predicted correct ratings).");
		pROC->add("[sparse-data]", "The filename of a sparse matrix where rows indicate users, columns indicate items, and elements in the matrix indicate ratings. Alternatively, you may specify the filename of a 3-column dense ARFF file where each row specifies a user-id, item-id, and rating.");
	}
	return pRoot;
}
...
UsageNode* makeCollaborativeFilterUsageTree()
{
	...
	{
		pClust->add("[n]", "The number of clusters to use.");
		...
		pOpts->add("-declineifnodata", "By default, this model will predict 0 if it has no basis to predict another value. If this flag is specified, it will predict UNKNOWN_REAL_VALUE instead, which will cause this prediction to not affect the cross-validation score.");
	}
	...
	{
		pInst->add("[k]", "The number of neighbors to use.");
		UsageNode* pOpts = pInst->add("<options>");
		pOpts->add("-pearson", "Use Pearson Correlation to compute the similarity between users. (The default is to use the cosine method.)");
		pOpts->add("-regularize [value]", "Add [value] to the denominator in order to regularize the results. This ensures that recommendations will not be dominated when a small number of overlapping items occurs. Typically, [value] will be a small number, like 0.5 or 1.5.");
		pOpts->add("-declineifnodata", "By default, this model will predict 0 if it has no basis to predict another value. If this flag is specified, it will predict UNKNOWN_REAL_VALUE instead, which will cause this prediction to not affect the cross-validation score.");
	}
	...
	{
		pInst->add("[intrinsic]", "The number of intrinsic (or latent) feature dims to use to represent each user's preferences.");
