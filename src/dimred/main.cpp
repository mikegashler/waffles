...
void unsupervisedBackProp(GArgReader& args)
{
	...
	while(args.size() > 0)
	{
		if(args.if_pop("-seed"))
			...
			pUBP->neuralNet()->addLayer(new GNeuralNetLayerClassic(FLEXIBLE_SIZE, args.pop_uint()));
	...
	pUBP->neuralNet()->addLayer(new GNeuralNetLayerClassic(FLEXIBLE_SIZE, FLEXIBLE_SIZE));
