...
GLayerConvolutional2D::GLayerConvolutional2D(size_t inputCols, size_t inputRows, size_t inputChannels, size_t kernelCols, size_t kernelRows, size_t kernelCount, size_t stride, size_t padding, GActivationFunction *pActivationFunction)
: m_inputCols(inputCols), m_inputRows(inputRows), m_inputChannels(inputChannels), m_kernelCols(kernelCols), m_kernelRows(kernelRows), m_kernelCount(kernelCount), m_stride(stride), m_padding(padding), m_outputRows((inputRows - kernelRows + 2 * padding) / stride + 1), m_outputCols((inputCols - kernelCols + 2 * padding) / stride + 1), m_bias(kernelCount), m_biasDelta(kernelCount), m_kernels(kernelCount, inputChannels * kernelRows * kernelCols), m_delta(kernelCount, inputChannels * kernelRows * kernelCols), m_activation(3, kernelCount * m_outputRows * m_outputCols), m_pActivationFunction(pActivationFunction ? pActivationFunction : new GActivationTanH())
{}

GLayerConvolutional2D::GLayerConvolutional2D(const GLayerConvolutional2D &upstream, size_t kernelCols, size_t kernelRows, size_t kernelCount, size_t stride, size_t padding, GActivationFunction *pActivationFunction)
: m_inputCols(upstream.outputCols()), m_inputRows(upstream.outputRows()), m_inputChannels(upstream.outputChannels()), m_kernelCols(kernelCols), m_kernelRows(kernelRows), m_kernelCount(kernelCount), m_stride(stride), m_padding(padding), m_outputRows((m_inputRows - kernelRows + 2 * padding) / stride + 1), m_outputCols((m_inputCols - kernelCols + 2 * padding) / stride + 1), m_bias(kernelCount), m_biasDelta(kernelCount), m_kernels(kernelCount, m_inputChannels * kernelRows * kernelCols + 1), m_delta(kernelCount, m_inputChannels * kernelRows * kernelCols), m_activation(3, kernelCount * m_outputRows * m_outputCols), m_pActivationFunction(pActivationFunction ? pActivationFunction : new GActivationTanH())
{}

GLayerConvolutional2D::GLayerConvolutional2D(size_t kernelCols, size_t kernelRows, size_t kernelCount, size_t stride, size_t padding, GActivationFunction *pActivationFunction)
: m_inputCols(FLEXIBLE_SIZE), m_inputRows(FLEXIBLE_SIZE), m_inputChannels(FLEXIBLE_SIZE), m_kernelCols(kernelCols), m_kernelRows(kernelRows), m_kernelCount(kernelCount), m_stride(stride), m_padding(padding), m_outputRows(0), m_outputCols(0), m_bias(kernelCount), m_biasDelta(kernelCount), m_kernels(kernelCount, 0), m_delta(kernelCount, 0), m_activation(3, FLEXIBLE_SIZE), m_pActivationFunction(pActivationFunction ? pActivationFunction : new GActivationTanH())
{}

GLayerConvolutional2D::GLayerConvolutional2D(GDomNode *pNode)
: m_inputCols(pNode->field("inputCols")->asInt()), m_inputRows(pNode->field("inputRows")->asInt()), m_inputChannels(pNode->field("inputChannels")->asInt()), m_kernelRows(pNode->field("kernelRows")->asInt()), m_kernelCols(pNode->field("kernelCols")->asInt()), m_kernelCount(pNode->field("kernelCount")->asInt()), m_stride(pNode->field("stride")->asInt()), m_padding(pNode->field("padding")->asInt()), m_outputRows(pNode->field("outputRows")->asInt()), m_outputCols(pNode->field("outputCols")->asInt()), m_bias(pNode->field("bias")), m_biasDelta(m_kernelCount), m_kernels(pNode->field("kernels")), m_delta(m_kernelCount, m_inputChannels * m_kernelRows * m_kernelCols), m_activation(3, m_kernelCount * m_outputRows * m_outputCols)
{
	m_pActivationFunction = GActivationFunction::deserialize(pNode->field("act_func"));
}
...
void GLayerConvolutional2D::resizeInputs(GNeuralNetLayer *pUpStreamLayer, GRand *pRand, double deviation)
{
	...
	m_bias.resize(m_kernelCount);
	m_biasDelta.resize(m_kernelCount);
	m_kernels.resize(m_kernelCount, m_inputChannels * m_kernelRows * m_kernelCols);
	m_delta.resize(m_kernelCount, m_inputChannels * m_kernelRows * m_kernelCols);
				...
				for(size_t z = 0; z < m_inputChannels; z++)
				{
					...
					for(size_t y = 0; y < m_kernelRows; y++)
					{
						...
						for(size_t x = 0; x < m_kernelCols; x++)
						{
							...
							if(inRow >= 0 && inRow < (int)m_inputRows && inCol >= 0 && inCol < (int)m_inputRows)
							{
...
void GLayerConvolutional2D::applyDeltas(double learningRate)
{
	...
	for(size_t i = 0; i < m_delta.rows(); i++)
		m_kernels[i].addScaled(learningRate, m_delta[i]);
}
...
{
	m_kernels.toVector(pOutVector);
	GVecWrapper(pOutVector + m_kernels.rows() * m_kernels.cols(), m_kernelCount).vec().put(0, m_bias);
	return countWeights();
}
...
size_t GLayerConvolutional2D::vectorToWeights(const double *pVector)
{
	...
	m_bias.put(0, GConstVecWrapper(pVector + m_kernels.rows() * m_kernels.cols(), m_kernelCount).vec());
	return countWeights();
}
