...
void G3DRegionGraph_makeGradientMagnitudeImage(const std::vector<GImage*>& in, std::vector<GImage*>& out)
{
	...
	for(int z = 0; z < in.size(); z++)
	{
		...
		for(int y = 0; y < pImage->height(); y++)
		{
			for(int x = 0; x < pImage->width(); x++)
			{
				...
				{
					int zz = std::max(0, std::min(in.size() - 1, z + dz));
					GImage* pImg = in[zz];
					for(int dy = -1; dy < 2; dy++)
					{
						int yy = std::max(0, std::min(pImg->height() - 1, y + dy));
						for(int dx = -1; dx < 2; dx++)
						{
							int xx = std::max(0, std::min(pImg->width() - 1, x + dx));
				...
				grad = std::max(grad, gx * gx + gy * gy + gz * gz);
				grad = std::max(grad, bx * bx + by * by + bz * bz);
